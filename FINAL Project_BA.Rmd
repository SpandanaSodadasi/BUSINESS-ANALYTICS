...
library(readr)
library(readxl)
HP_train <- read.csv("C:/Users/spand/Downloads/House_Prices.csv")
HP_test <- read_excel("C:/Users/spand/Downloads/BA-Predict.xlsx")
```
...
```{r}
Model <- lm(SalePrice ~ ., data = HP_train)
...
```{r}
Model_1 <- lm(SalePrice ~ LotArea + OverallQual + YearRemodAdd + BsmtFinSF1 + BedroomAbvGr +  TotRmsAbvGrd + Fireplaces + GarageArea, data = HP_train)
summary(Model_1)
```
...
```{r}
set.seed(1)
train.index <- sample(row.names(HP_train))
test.index <- sample(row.names(HP_test)) 
```
...
```{r}
Model_2 <- lm(SalePrice ~ LotArea + OverallQual + YearRemodAdd + BsmtFinSF1 + BedroomAbvGr +  TotRmsAbvGrd + Fireplaces + GarageArea, data = HP_train)
summary(Model_2)
Predicted_value_1 <- predict(Model_2, data.frame(HP_test))
Predicted_value_1
```

This code builds a linear regression model (Model_2) to predict SalePrice based on various features that influence the output variable using the training dataset (HP_train). The summary function provides detailed information about the model's coefficients and statistical significance. The code then generates predicted SalePrice values for the test dataset (HP_test) using the model and stores them in Predicted_value_1. 

#Calculating the RMSE Value. 
```{r}
RMSE_Prediction_1 <- RMSE(Predicted_value_1, HP_test$SalePrice)
RMSE_Prediction_1
```
...
```{r}
Model_3 <- rpart(SalePrice ~., data = HP_train, method = 'anova', control = rpart.control(minsplit = 60))
summary(Model_3)
```
...
#Predicting the price of a house based on selected predictors.
```{r}
Model_4 <- rpart(SalePrice ~ OverallQual + GarageArea + YearBuilt + BsmtFinSF1 + YearRemodAdd , data = HP_train, method = 'anova', control = rpart.control(minsplit = 60))
summary(Model_4)
fancyRpartPlot(Model_4)
Predicted_value_2 <- predict(Model_4, data.frame(HP_test))
Predicted_value_2
```

The code constructs a decision tree model, to predict SalePrice using the HP_train dataset with the selected features. The model's summary and a visual representation of the decision tree are generated. Finally, the Predicted_value_2 variable holds the predicted SalePrice values for the HP_test dataset. 

#Calculating the value RMSE and Adjusted R-squared value value.
```{r}
Observations <- nrow(HP_train)  # number of observations
Predictors <- length(coefficients(Model_4)) - 1  # number of predictors (excluding intercept)

#Calculating R-squared Value
SSE <- sum((Predicted_value_2 - HP_train$SalePrice)^2)
SST <- sum((mean(HP_train$SalePrice) - HP_train$SalePrice)^2)
R2 <- 1 - (SSE / SST)
round(R2,3)
...
RMSE_Prediction_2 <- RMSE(Predicted_value_2, HP_test$SalePrice)
RMSE_Prediction_2
```
...
```{r}
HP_train$OverallQual <- as.factor(ifelse(HP_train$OverallQual >= 7, '1', '0'))
levels(HP_train$OverallQual)
HP_test$OverallQual <- as.factor(ifelse(HP_test$OverallQual >= 7, '1', '0'))
levels(HP_test$OverallQual)
```

#Running the logistic regression model to perform feature selection.
```{r}
Model_5 <- glm(OverallQual ~. , data = HP_train, family = 'binomial')
summary(Model_5)
```
...
```{r}
Model_6 <- glm(OverallQual ~ LotArea + BsmtFinSF1 + BedroomAbvGr + SalePrice, data = HP_train, family = 'binomial')
summary(Model_6)
Predicted_probs <- predict(Model_6, newdata = HP_test, type = "response")
cutoff <- 0.5
class_prediction <- ifelse(Predicted_probs > cutoff, "1", "0")
class_prediction <- as.factor(class_prediction)
HP_test$OverallQual <- as.factor(HP_test$OverallQual)
conf_matrix <- confusionMatrix(class_prediction, HP_test$OverallQual, positive = "1")
print(conf_matrix)
```
